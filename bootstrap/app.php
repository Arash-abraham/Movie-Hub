<?php
    if (!function_exists('dd')) {
        function full_export($var, $depth = 0, $max_depth = 5, $max_items = 50, $max_length = 1000) {
            ...
            if ($type === 'object' && in_array($hash, $processed)) {
                return '**RECURSION**';
            }
            if ($type === 'object') {
                $processed[] = $hash;
        ...
        function dd(...$args): never
        {
            ...
            echo '<style>
                ...
                *{margin:0;padding:0;box-sizing:border-box}
                ...
                pre{margin:0;white-space:pre-wrap;word-wrap:break-word;font-size:12px;line-height:1.4;font-family:"Fira Code","Courier New",monospace}
                .string{color:#4ec9b0}
            ...
            if ($total_size > 1000000) {
                echo '<div class="warning">⚠️ Large data detected. Display may be truncated.</div>';
            }
            ...
            foreach ($args as $i => $val) {
                try {
                    ...
                    $exp = preg_replace([
                        ...
                        // Array indexes
                        '/\[(\d+)\]/',
                        // Object keys (without visibility)
                        '/&quot;([^#+-][^&]*)&quot;\s*=&gt;/',
                        // Public properties
                        '/&quot;\+([^&]*)&quot;\s*=&gt;/',
            ...
            echo '<script>
                    ...
                    // Auto-close when data is large
                    if (' . ($total_size > 500000 ? 'true' : 'false') . ') {
                        setTimeout(() => {
                            document.querySelectorAll(".variable").forEach(detail => {
                                detail.open = false;
                            });
                        }, 100);
                    }
